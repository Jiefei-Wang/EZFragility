...
The function `calcAdjFrag` computes the fragility matrix for the given iEEG time series. Computing Fragility Matrix can take a bit while, so we provide a parallel computing option to speed up the computation (see below).
```{r compute fragility, eval=FALSE}
window <- 250
step <- 125
pt01Fragm1sp2s <- calcAdjFrag(ieegts = pt01Epochm1sp2s, window = window, step = step)
```

To speed up the computation, you can enable parallel computing by passing `parallel = TRUE` to the function. The parallel feature is provided by the `foreach` package. You need to have a parallel backend registered in your R session beforehand. For example, you can use the `doParallel` package to register a parallel backend.
```{r}
library(doParallel)
...
# Stop the parallel backend (optional)
stopImplicitCluster()
```
