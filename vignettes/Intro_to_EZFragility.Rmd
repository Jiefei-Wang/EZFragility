...
```
Equivalently, we can use `visuIEEGData(epoch  = pt01EcoG[display, ])` to visualize the data for the specified electrodes. You can also specify the range of time points to display. For example, to display the first 100 time points, you can use:
```{r}
...
```{r}
pt01 <- dl$Retrostudy_subpt01_1
pt01Clipped <- crop(pt01, start=-10, end=10)

## sozIndex is the index of the electrodes we assume are in the SOZ
sozIndex <- which(rowData(pt01)$soz==TRUE)
display <- c(sozIndex, 77:80)
visuIEEGData(epoch = pt01Clipped[display, ])
```
