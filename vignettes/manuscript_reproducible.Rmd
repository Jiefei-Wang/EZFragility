...
## Bandpass filter voltage between 0.5 and Nyquist frequency with fourth-order Butterworth filter
```{r}

pt01sz1Filt<-FourthOrderButterworthFilter(pt01sz1Clipped)
pt26sz1Filt<-FourthOrderButterworthFilter(pt26sz1Clipped)

```

To visualize patient 01 seizure 1:
```{r}
pt01sozName <- rownames(pt01sz1Filt)[rowData(pt01sz1Filt)$soz]
pt01Display <- c(pt01sozName, "MLT1", "MLT2", "MLT3", "MLT4")
pt01sz1Reordered <- pt01sz1Filt[pt01Display, ]
...
```{r}
pt26sozName <- rownames(pt26sz1Filt)[rowData(pt26sz1Filt)$soz]
...
pt26Display <- c(
    ...
    excludedElectrodes,
    pt26sozName[17:18])
pt26sz1Reordered <- pt26sz1Filt[pt26Display, ]
...
windowNum <- 250
step <- 125
pt01sz1Frag <- calcAdjFrag(epoch = pt01sz1Filt, window = windowNum, step = step, parallel = TRUE, nSearch=100L, progress = FALSE)

pt26sz1Frag <- calcAdjFrag(epoch = pt26sz1Filt, window = windowNum, step = step, parallel = TRUE, nSearch=100L, progress = FALSE)
